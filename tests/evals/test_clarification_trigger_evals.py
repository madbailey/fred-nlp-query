...
_CASE_PATH = Path(__file__).with_name("clarification_trigger_cases.json")


def test_live_clarification_trigger_eval_cases(request: pytest.FixtureRequest) -> None:
    ...
    write_scorecard(
        request,
        title="Clarification trigger eval scorecard",
        model=model,
    ...
    if failures:
        ...
        pytest.fail("Clarification trigger eval failures:\n" + "\n\n".join(failure_lines))
