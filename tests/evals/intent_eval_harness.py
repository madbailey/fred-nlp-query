...
def evaluate_case(parser: OpenAIIntentParser, case: dict[str, Any]) -> EvalResult:
...
def write_scorecard(
    ...
    *,
    ...
    lines = [
        f"{title} | model={model} | reasoning_effort={reasoning_effort}",
        *[
    ...
    *,
    ...
    if path.suffix.lower() == ".json":
        if suite == "intent":
            ...
            target_path = path.with_name(f"{path.stem}-{suite}{path.suffix}")
    else:
        target_path = path / f"{suite}.json"
...
def build_live_parser(request: pytest.FixtureRequest) -> tuple[OpenAIIntentParser, str, str]:
